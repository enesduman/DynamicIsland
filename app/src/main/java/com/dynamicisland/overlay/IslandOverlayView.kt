...
class IslandOverlayView(
    ...
    private fun setupTouch() {
        ...
        // Onemli: container yerine tum view'da touch yakala
        // Boylece ust kisimda bile calisir
        this.setOnTouchListener { v, event ->
            // Container icinde mi kontrol et
            val loc = IntArray(2)
            container.getLocationOnScreen(loc)
            val inX = event.rawX >= loc[0] && event.rawX <= loc[0] + container.width
            val inY = event.rawY >= loc[1] && event.rawY <= loc[1] + container.height

            if (inX && inY) {
                gestureDetector.onTouchEvent(event)
                true
            } else if (cur.expanded) {
                // Expanded iken disari tikla -> kapat
    ...
    private fun vibratePattern() {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                vib.vibrate(VibrationEffect.createWaveform(longArrayOf(0, 15, 40, 25, 40, 40), -1))
            }
        } catch (_: Exception) {}
    }
    ...
    private fun updateGlow(color: Int) {
        ...
        if (color != currentGlow) {
            ...
            glowView.alpha = 0f
            glowView.animate().alpha(0.6f).setDuration(300).start()
        }
        val lp = glowView.layoutParams as LayoutParams
        val clp = container.layoutParams
        lp.width = clp.width + dp(6f)
        lp.height = clp.height + dp(6f)
        lp.topMargin = (container.layoutParams as LayoutParams).topMargin - dp(3f)
    ...
    private fun cRecord() {
        compactV.gravity = Gravity.CENTER_VERTICAL
        compactV.addView(dot(0xFFFF3B30.toInt(), 8))
        compactV.addView(spacer())
    ...
    private fun eMusic(m: MusicState) {
        ...
        val ctrls = hLay(Gravity.CENTER).apply {
        ...
        ctrls.addView(ctrlBtn(if (m.isPlaying) "\u23F8" else "\u25B6\uFE0F", true) { mt?.togglePlayPause() }.apply {
            (layoutParams as LinearLayout.LayoutParams).marginStart = dp(14f)
            (layoutParams as LinearLayout.LayoutParams).marginEnd = dp(14f)
        })
    ...
    private fun eCall(c: CallState) {
        ...
        lay.addView(TextView(context).apply {
            text = c.callerName.take(2).uppercase(); textSize = 14f
        ...
        lay.addView(tv(c.callerName.ifEmpty { c.callerNumber.ifEmpty { "Bilinmeyen" } }, 13f, Color.WHITE,
            Typeface.create(Typeface.DEFAULT, Typeface.BOLD)).apply {
            gravity = Gravity.CENTER; (layoutParams as? LinearLayout.LayoutParams)?.topMargin = dp(3f)
        })
        lay.addView(tv(if (c.isActive) fmtD(c.durationSeconds) else if (c.isIncoming) "Gelen Arama" else "Ariyor...",
            11f, if (c.isActive) 0xFF4CD964.toInt() else 0xFF999999.toInt(), Typeface.MONOSPACE).apply {
        ...
        if (c.isIncoming) {
            btns.addView(actionBtn("\u2714", 0xFF4CD964.toInt()) {
                try { context.startActivity(context.packageManager.getLaunchIntentForPackage("com.android.dialer")?.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)) } catch (_: Exception) {}
            })
            btns.addView(View(context).apply { layoutParams = LinearLayout.LayoutParams(dp(20f), 1) })
            btns.addView(actionBtn("\u2716", 0xFFFF3B30.toInt()) {
                try { (context.getSystemService(Context.TELECOM_SERVICE) as TelecomManager).endCall() } catch (_: Exception) {}
            })
        } else if (c.isActive) {
    ...
    private fun eCharge(ch: ChargingState) {
        ...
        val r = hLay(Gravity.CENTER)
        r.addView(tv("\u26A1", 20f))
        r.addView(tv("${ch.level}%", 24f, 0xFF4CD964.toInt(), Typeface.create(Typeface.DEFAULT, Typeface.BOLD)).apply {
            setPadding(dp(4f), 0, 0, 0)
        })
        lay.addView(r)
        lay.addView(ProgressBar(context, null, android.R.attr.progressBarStyleHorizontal).apply {
            layoutParams = LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, dp(4f)).apply { topMargin = dp(6f) }
            max = 100; progress = ch.level
        })
        lay.addView(tv(if (ch.isFast) "Hizli sarj" else "Sarj ediliyor", 10f, 0xFF999999.toInt()).apply {
            gravity = Gravity.CENTER; (layoutParams as? LinearLayout.LayoutParams)?.topMargin = dp(3f)
        })
        expandedV.addView(lay)
    }
    ...
    private fun eNotif(n: NotificationInfo?) {
        if (n == null) return
        val lay = hLay(Gravity.CENTER_VERTICAL).apply {
        ...
        if (n.appIcon != null) lay.addView(ImageView(context).apply {
            layoutParams = LinearLayout.LayoutParams(dp(28f), dp(28f)).apply { marginEnd = dp(6f) }
            setImageBitmap(n.appIcon)
        }) else lay.addView(View(context).apply {
            layoutParams = LinearLayout.LayoutParams(dp(28f), dp(28f)).apply { marginEnd = dp(6f) }
            background = GradientDrawable().apply { cornerRadius = dp(6f).toFloat(); setColor(n.color) }
        })
        val tc = LinearLayout(context).apply {
            orientation = LinearLayout.VERTICAL
            layoutParams = LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f)
        ...
        lay.addView(tc)
        lay.addView(tv("\u2715", 12f, 0xFF666666.toInt()).apply {
    ...
    private fun eTimer(t: TimerState) {
        ...
        lay.addView(tv(display, 22f, 0xFFFF9500.toInt(), Typeface.MONOSPACE).apply {
            gravity = Gravity.CENTER; (layoutParams as? LinearLayout.LayoutParams)?.topMargin = dp(2f)
        })
        lay.addView(actionBtn("\u23F9 Durdur", 0xFFFF3B30.toInt()) { TimerManager.stop() }.apply {
    ...
    private fun eNet(n: NetSpeedState) {
        ...
        dl.addView(tv("\u2193 ", 12f, 0xFF4CD964.toInt()))
        dl.addView(tv(n.downloadSpeed, 15f, 0xFF4CD964.toInt(), Typeface.create(Typeface.DEFAULT, Typeface.BOLD)))
        lay.addView(dl)
        val ul = hLay(Gravity.CENTER).apply { (layoutParams as LinearLayout.LayoutParams).topMargin = dp(2f) }
        ul.addView(tv("\u2191 ", 12f, 0xFFFF6B35.toInt()))
        ul.addView(tv(n.uploadSpeed, 15f, 0xFFFF6B35.toInt(), Typeface.create(Typeface.DEFAULT, Typeface.BOLD)))
        lay.addView(ul)
        expandedV.addView(lay)
    }
    ...
    private fun eCamMic(ind: SystemIndicators) {
        ...
        if (ind.cameraInUse) {
            ...
            r.addView(tv("  Kamera", 12f, 0xFF4CD964.toInt()))
            lay.addView(r)
        }
        if (ind.micInUse) {
            val r = hLay(Gravity.CENTER).apply { (layoutParams as LinearLayout.LayoutParams).topMargin = dp(3f) }
            r.addView(dot(0xFFFF9500.toInt(), 10))
            r.addView(tv("  Mikrofon", 12f, 0xFFFF9500.toInt()))
            lay.addView(r)
        }
        expandedV.addView(lay)
    }
    ...
    private fun eRecord() {
        val lay = vLay(Gravity.CENTER)
        val r = hLay(Gravity.CENTER)
        r.addView(dot(0xFFFF3B30.toInt(), 10))
        r.addView(tv("  Ekran Kaydediliyor", 12f, 0xFFFF3B30.toInt(), Typeface.create(Typeface.DEFAULT, Typeface.BOLD)))
        lay.addView(r)
        expandedV.addView(lay)
    }
    ...
    private fun eBt(ind: SystemIndicators) {
        ...
        lay.addView(tv(if (ind.bluetoothConnected) "Bagli" else "Bagli degil", 10f,
            if (ind.bluetoothConnected) 0xFF4CD964.toInt() else 0xFF999999.toInt()).apply {
            gravity = Gravity.CENTER; (layoutParams as? LinearLayout.LayoutParams)?.topMargin = dp(2f)
        })
        expandedV.addView(lay)
    }
    ...
    private fun animTo(expand: Boolean, s: IslandState) {
        ...
        // Genisletilmis: ekranin %60'i
        val tw = if (expand) (sw * 0.6).toInt()
                 else if (s.mode != IslandMode.IDLE) dp(idleW.toFloat() + 50f)
                 else dp(idleW.toFloat())
        val th = if (expand) when (s.mode) {
            IslandMode.MUSIC -> dp(110f)
            IslandMode.CALL -> dp(130f)
            IslandMode.CHARGING -> dp(85f)
            IslandMode.NOTIFICATION -> dp(55f)
            IslandMode.TIMER -> dp(90f)
            IslandMode.NET_SPEED -> dp(85f)
            IslandMode.NAVIGATION -> dp(75f)
            IslandMode.CAMERA_MIC -> dp(55f)
            IslandMode.SCREEN_RECORD -> dp(45f)
            IslandMode.BLUETOOTH -> dp(60f)
            else -> dp(70f)
        ...
        ValueAnimator.ofInt(lp.width, tw).apply {
            duration = 350; interpolator = OvershootInterpolator(0.8f)
        ...
        // Expanded iken touch alani genislet
        if (expand) {
            val olp = this.layoutParams
            if (olp != null) {
                olp.height = th + dp(20f)
                this.layoutParams = olp
            }
        } else {
            val olp = this.layoutParams
            if (olp != null) {
                olp.height = android.view.ViewGroup.LayoutParams.WRAP_CONTENT
                this.layoutParams = olp
            }
        }
    }
    ...
    private fun hLay(g: Int): LinearLayout = LinearLayout(context).apply {
        orientation = LinearLayout.HORIZONTAL; gravity = g
    ...
    private fun ctrlBtn(t: String, big: Boolean, click: () -> Unit) = TextView(context).apply {
        text = t; textSize = if (big) 18f else 13f; gravity = Gravity.CENTER
        val s = if (big) dp(36f) else dp(28f); layoutParams = LinearLayout.LayoutParams(s, s)
        if (big) { background = GradientDrawable().apply { shape = GradientDrawable.OVAL; setColor(Color.WHITE) }; setTextColor(Color.BLACK) }
        setOnClickListener { vibrateLight(); click()
    ...
    private fun actionBtn(t: String, bgColor: Int, click: () -> Unit) = TextView(context).apply {
        text = t; textSize = 13f; gravity = Gravity.CENTER; setTextColor(Color.WHITE)
    ...
    private fun fmtMsFull(ms: Long): String {
        val t = (ms / 1000).toInt(); val m = t / 60; val s = t % 60; val cs = ((ms % 1000) / 10).toInt()
