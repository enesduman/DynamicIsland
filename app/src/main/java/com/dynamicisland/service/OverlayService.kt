...
class OverlayService : LifecycleService() {
    companion object {
        ...
        const val ACT_START = "com.dynamicisland.START"; const val ACT_STOP = "com.dynamicisland.STOP"
    ...
    private var wm: WindowManager? = null; private var ov: IslandOverlayView? = null
    private var mt: MediaSessionTracker? = null; private var ct: CallStateTracker? = null

    override fun onStartCommand(i: Intent?, f: Int, id: Int): Int { super.onStartCommand(i, f, id)
        if (i?.action == ACT_STOP) { stopSelf(); return START_NOT_STICKY }
        startForeground(NID, notif()); setup(); trackers(); observe(); return START_STICKY }
    ...
    private fun setup() { if (ov != null) return
        wm = getSystemService(WINDOW_SERVICE) as WindowManager
        mt = MediaSessionTracker(this); ov = IslandOverlayView(this, mt)
    ...
    private fun notif(): Notification {
        ...
        return NotificationCompat.Builder(this, CID).setContentTitle("Dynamic Island aktif")
            .setContentText("Dokunarak ayarları açın").setSmallIcon(R.drawable.ic_island)
